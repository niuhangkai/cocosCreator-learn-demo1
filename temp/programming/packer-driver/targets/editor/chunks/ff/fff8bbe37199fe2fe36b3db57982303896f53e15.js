      ...
      _export("Player", Player = (_dec = ccclass('Player'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property({
        ...
        singleShoot(deltaTime) {
          ...
          if (this.shootTimer >= this.frequency) {
            ...
            const bullet1 = instantiate(this.bulletPrefab); // 添加到场景中
        ...
        doubleShoot(deltaTime) {
          ...
          if (this.shootTimer >= this.frequency) {
            ...
            const bullet2 = instantiate(this.bulletPrefab2); // 添加到场景中
        ...
        onTouchMove(event) {
          ...
          const p = this.node.position; // Vec3 是 cocos 的向量类

          let targetPos = new Vec3(p.x + event.getDeltaX(), p.y + event.getDeltaY(), p.z); // 限制移动范围
        ...
        onBeginContact(selfCollider, otherCollider, contact) {
          ...
          const reward = otherCollider.getComponent(_crd && Reward === void 0 ? (_reportPossibleCrUseOfReward({
            error: Error()
          }), Reward) : Reward);
